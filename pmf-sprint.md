# Marketplace Flip Alerts — PMF Sprint

## Goal
Get to first dollars and first real usage feedback with the smallest possible product.

## Week 1 success definition
Any of these counts as meaningful progress:
- 1 paying pilot user
- 3 serious pilot conversations
- 5 strong objections that clearly sharpen the offer
- 1 user who asks for faster delivery, more categories, or broader geographic coverage

## Daily priorities
1. Tighten offer language
2. Get the page in front of real resellers
3. Capture objections and category demand
4. Standardize delivery format
5. Kill anything that does not increase pilot conversion

## Core metric stack
- pilot applications
- reply rate
- paid pilots
- requested categories
- requested metros
- objections by frequency

## Operational rules
- No broad software build before paid signal.
- No feature work that does not improve conversion or delivery quality.
- Stay on one wedge: profitable sourcing alerts.
- Use concierge/manual operations to learn fast.

## Likely next build after signal
- Telegram alert bot
- saved-search automation
- category presets
- scoring heuristics
- dashboard only if delivery volume justifies it
